function [Q_mat] = get_gibbs_Q_noall0(Q_mat, Y_arr, beta_mat, beta0, A_mat, gamma_q, sig2_beta, sig2_pseudo)
...
for k=1:K
    ...
    % constraint begins: do not allow any row to be all zero
    all0_rows = find(sum(Q_mat,2) == 0);
    if ~isempty(all0_rows)
        for aa=1:length(all0_rows)
            jj = all0_rows(aa);
            Q_mat(jj, :) = Q_mat_old(jj, :);
        end
    end
    % constraint ends here
