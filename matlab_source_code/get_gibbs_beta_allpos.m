function [beta_mat, beta0] = get_gibbs_beta_allpos(beta_mat, beta0, Y_arr, PG_param, C_ij_minus_c,...
    A_mat, Q_mat, sig2_beta, sig2_pseudo, var_beta0, mu_beta0)

% This function samples all the active beta constrained to be positive
...
for c=1:d-1
    ...
    for j=1:p
        ...
        if num_k==1
            ...
            if ~isinf(beta_candidate) && ~isnan(beta_candidate)
                beta_mat(j, Q_mat(j,:)==1, c) = beta_candidate;
            else
                % beta_mat(j, Q_mat(j,:)==1, c) = 0;
                beta_mat(j, Q_mat(j,:)==1, c) = normrnd(0, sqrt(Sigma_jc), num_k);
            end

        else
            try
                % beta_mat(j, Q_mat(j,:)==1, c) = mvnrnd(mu_jc, Sigma_jc, 1);
                beta_mat(j, Q_mat(j,:)==1, c) = rmvnrnd(mu_jc, Sigma_jc, 1,...
                    [-eye(num_k);eye(num_k)], [zeros(num_k,1); Inf*ones(num_k,1)]);
            catch
