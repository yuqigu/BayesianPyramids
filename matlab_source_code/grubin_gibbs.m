function [Q_mat_arr, beta_mat_arr, beta0_arr, Bern_K_arr, tau_arr, K_star_arr, ...
    Q_active_all, beta_mattil_active, Bern_K_permute2, tau_permute2] ...
    = rev_grubin_gibbs(Y_arr, K, B, nrun, burn, thin, alpha0)

% This function runs the Gibbs sampler for the SGLCA with CSP prior
% Gelman-Rubin diagnostics: trait permutation after collecting the
% posterior samples


% Used to have name "rev_grubin_csp_pos_randini"
...
for c = 1:d-1
    pd0 = makedist('Normal', 'mu', mu_beta0(c), 'sigma', sqrt(var_beta0(c)));
    beta0(:, c) = random(truncate(pd0,-6,-3), 1, 1);
end
...
Q_mat_g = squeeze(mean(Q_mat_arr(:,:,burn+1:thin:end), 3));
sig2_beta_g = squeeze(mean(sig2_beta_arr(:,:,burn+1:thin:end), 3));
Bern_K_g = squeeze(mean(Bern_K_arr(:,:,burn+1:thin:end), 3));
tau_g =  squeeze(mean(tau_arr(:,burn+1:thin:end), 2));
beta0_g = squeeze(mean(beta0_arr(:,:,burn+1:thin:end), 3));
beta_mat_g = squeeze(mean(beta_mat_arr(:,:,:,burn+1:thin:end), 4));
beta_mattil_g = squeeze(mean(beta_mat_arr(:,:,:,burn+1:thin:end) ...
    .* reshape(Q_mat_arr(:,:,burn+1:thin:end), [p K 1 L]), 4));


% -- find the active traits -- %
[sort_mean_var, rank_mean_var] = sort(mean(sig2_beta_g, 2), 'descend');
%
[~, K_act] = max(sort_mean_var(1:end-1) - sort_mean_var(2:end));
% K_active(g) = K_act;
%
active_traits = zeros(K,1);
active_traits(rank_mean_var(1:K_act)) = 1;

find_active = find(active_traits);

Q_binary_pomean = (Q_mat_g(:, active_traits == 1) > 0.5);
...
% Q_active_all, beta_mattil_active, Bern_K_permute2, tau_permute2
Q_active_all = Q_mat_g(:, permute_augment);

beta_mat_active = beta_mat_g(:, permute_augment, :);
beta_mattil_active = beta_mattil_g(:, permute_augment, :);

Bern_K_permute = Bern_K_g(permute_augment, :);

% second permutation of tau and Bern_K (eta)
if Bern_K_permute(1,1) > Bern_K_permute(1,2)
    Bern_K_permute2 = Bern_K_permute;
    tau_permute2 = tau_g;
else
    Bern_K_permute2 = [Bern_K_permute(:,2), Bern_K_permute(:,1)];
    tau_permute2 = [tau_g(2); tau_g(1)];
end
% -- permutation of pomean here -- %


%  -- permutation of MCMC samples -- %
% Q_mat_arr, beta_mat_arr, beta0_arr, Bern_K_arr, tau_arr
Q_mat_arr = Q_mat_arr(:,permute_augment,:);
beta_mat_arr = beta_mat_arr(:,permute_augment,:,:);
Bern_K_arr = Bern_K_arr(permute_augment,:,:);

% second permutation of tau and Bern_K (eta)
if Bern_K_permute(1,1) <= Bern_K_permute(1,2)
    Bern_K_arr = Bern_K_arr(:,[2 1],:);
    tau_arr = tau_arr([2 1],:);
end
