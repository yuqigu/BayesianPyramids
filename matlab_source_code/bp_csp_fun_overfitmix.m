function [Q_mat_arr, beta_mat_arr, beta0_arr, A_mat_arr, Bern_K_arr, tau_arr,...
    ...
    = bp_csp_fun_overfitmix(Y_arr, K, B, nrun, alpha0)

% This function runs the Gibbs sampler for Bayesian Pyramid with CSP prior
% with an overfitted mixture for the deeper latent layer. B is the upper
% bound for the number of categories the deepest z can take
...
tau_temp = gamrnd(dir_param, 1, [1 B]); % ones(B,1)/B; % C * 1
...
for c = 1:d-1
    ...
    beta0(:, c) = random(truncate(pd0,-5,-1), 1, 1);
end
...
%     end
    ...
    tau_temp = gamrnd(dir_param + sum(z_tau_mat, 1), 1); 
    tau = tau_temp/sum(tau_temp);
