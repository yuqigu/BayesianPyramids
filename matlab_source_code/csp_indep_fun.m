function [Q_mat_arr, beta_mat_arr, beta0_arr, A_mat_arr,...
    sig2_beta_arr, K_star_arr, z_beta_vec_arr] ...
    = csp_indep_fun(Y_arr, K, nrun, alpha0)

% This function gives the Gibbs sampler for one independent binary latent layer
% with the CSP prior
...
sig2_inf = 0.1;
% alpha0 = 5;
% z_beta_mat_cat = zeros(K,K,d);
% v_beta_cat = zeros(K,d-1);
% w_beta_cat = zeros(K,d-1);
...
%     end
    ...
    eta_K = betarnd(1 + sum(A_mat,1), 1 + sum(1-A_mat,1));
    eta_K_err(:,ii) = eta_K;

    % (7) Gibbs sampling for A_mat
    % A_mat = get_gibbs_A(A_mat, Y_arr, beta_mat, beta0, Q_mat, Bern_K, z_tau_mat);
    A_mat = get_gibbs_A_indep(A_mat, Y_arr, beta_mat, beta0, Q_mat, eta_K);
...
% % -- option 1: if we look for the permutation of latent traits:
