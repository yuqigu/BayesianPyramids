function [] = promoter_csp(K, alpha0)
...
for ii=burn+1:thin:nrun
    ...
    Q_rep_g = (Q_mat_pomean > 0.5);
    penalty_eff(:,ind) = loglik(Y_arr, Q_rep_g, beta_mat_pomean, beta0_pomean, ...
        Bern_K_pomean, tau_pomean, A_mat_arr(:,:,ind), z_tau_mat_arr(:,:,ind));
end
...
filename = strcat('promoter_CSPbeta_n', num2str(n), 'p', num2str(p),...
    '_Kini', num2str(K), '_d', num2str(d), '_alpha', num2str(alpha0), '.mat');
