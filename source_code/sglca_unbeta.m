...
% parpool setup
parpool(rep);
pctRunOnAll load('datasets_rep25_K4d4_n2000p20.mat')
...
parfor g=1:rep
    ...
    fprintf('Initialization for replicate %d completed\n', g);
...
%                 end
    ...
    % Q-matrix, permuted
    Q_mat_rep(:,:,g) = Q_pm_raw(:, permute_vec);
    
    
    % beta_mat, permuted
    beta_mat_pomean_rep(:,:,:,g) = mean(beta_mat_arr, 4);
    
    beta_mattil_pomean_rep(:,:,:,g) = mean(beta_mat_tilde_arr, 4);
    ...
    for rr=1:nrun
        DIC_part2(rr) = loglik(Y_arr, Q_mat_rep(:,:,g), beta_mat_pomean_rep(:,:,:,g), beta0_pomean_rep(:,:,g), ...
             Bern_K_pomean_rep(:,:,g), tau_pomean_rep(:,g), A_mat_arr(:,:,rr), z_mat_arr(:,:,rr));
    end

    DIC_vec = - 4/eff_sample/n * DIC_part1 + 2/eff_sample/n * DIC_part2;
    
    DIC_rep(g) = mean(DIC_vec(burn+1:thin:nrun));
    

end

filename = strcat('msimu_DIC_Kini_', num2str(K), 'd', num2str(d), ...
    '_n', num2str(n), 'p', num2str(p), '_rep', num2str(rep), '.mat');
