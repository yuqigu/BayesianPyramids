function [Q_mat_arr, beta_mat_arr, beta0_arr, A_mat_arr, Bern_K_arr, tau_arr, z_mat_arr, sig2_beta_arr] ...
    = sglca_saturated_fun(Y_now_arr, K, B, nrun, burn, thin)

% removes the deeper layer model when K is small
...
alpha_nu = ones(2^K, 1);
nu_temp = gamrnd(alpha_nu, 1); nu = nu_temp/sum(nu_temp);
A_mat = A_all(mnrnd(1, nu, n) * (1:2^K)', :);
Lambda_arm = zeros(2^K, p, d);
% SATURATED ENDS
...
%                 beta_mat(j, Q_mat(j,:)==0, c) = random(truncate(pd_pseudo,0,Inf), [1 sum(Q_mat(j,:)==0)]);
    ...
    for j=1:p
        ...
        nume_temp = exp(beta0(j,:) + A_all * squeeze(beta_mat(j,:,:)));
        denom_temp = 1 + sum(nume_temp, 2);
        Lambda_arm(:,j,1:end-1) = nume_temp ./ denom_temp;

    end
    Lambda_arm(:,:,end) = 1 - sum(Lambda_arm(:,:,1:end-1),3);
